...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const model of modelsToTry) {
      ...
      // Fail fast per model to avoid hitting the Supabase hard execution limit (e.g. 60s)
      const timeoutId = setTimeout(() => controller.abort(new Error("Timeout (15s)")), 15000);
      ...
      try {
        ...
        console.warn(`Model ${model} threw error:`, lastErr);
      }
    }
